...
Yn.SwitchButtonView = Em.ContainerView.extend(Yn.Btap, {
  ...
  fireChange: function(value) {
    ...
    this.triggerEvent( this.get('switchAction'), value );

  },

  bTap: function() {

    var newValue = !this.get('isOn');
    this.set('isOn', newValue );
	...
	button: Em.ContainerView.extend({
		...
		child: Em.View.extend({
			...
			isEnabled: Em.computed(function() {
				...
				return this.get('parentView.parentView.isEnabled');
          ...
          if ( percent !== 0 && percent !== 100 ) {
			...
			_isOnChanged: Em.observer(function() {
				...
				var percent = this.get('parentView.parentView.isOn') ? 100 : 0;
        ...
        //this._percentChanged();
			...
			didInsertElement: function(){
				...
				this.switchButtonView = this.get('parentView.parentView');
        ...
        var oldIsOn = this.get('parentView.parentView.isOn');
        var newIsOn = percent === 100;

				this.setPath('parentView.parentView.isOn', newIsOn );

        if ( oldIsOn !== newIsOn ) {
          this.get('parentView.parentView').fireChange(newIsOn);
        }
