...
Yn.NavigationView = Em.ContainerView.extend({
  ...
  _move: function(forward, disableTransition) {
  ...
  popView: function(disableTransition) {
    ...
    if ( !this.get('isMoving') ) {
      ...
      this.set('position', this.get('position')-1 );
      this.set('isMoving', true );
      this._move(false, disableTransition);
    }

  },


  popToRootView: function(animated, fn) {

    console.log('popToRootView');
    ...
    if ( !animated ) {
        ...
        console.log( 'contains ' + childs.contains(rootView)  );
        if ( childs.contains(rootView) ) {

          //this.get('element').style.webkitTransform = 'translate3d(0px,0,0)';
          ...
          while(view!==rootView) {
            ...
            console.log( 'remoginv  ' + view.get('elementId')  );

            if ( childs.contains(view) ) {

              console.log( 'andObject  ' + view.get('elementId')  );
  ...
  popToView: function(view, animated, fn) {

    console.log('view ' + view.get('elementId') );
    ...
    length = this.views.get('length');
    lastView = this.views[length-1];
    console.log('last view ' + lastView.get('elementId') );
    ...
    while (lastView !== view) {
      ...
      console.log('last view ' + lastView.get('elementId') + ' length ' + length);
